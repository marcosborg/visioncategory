        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.pageForm.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.page-forms.update", [$pageForm->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.pageForm.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $pageForm->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone" id="phone" value="{{ old('phone', $pageForm->phone) }}" required>
                            @if($errors->has('phone'))
                        ...
                        <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="email" name="email" id="email" value="{{ old('email', $pageForm->email) }}" required>
                            @if($errors->has('email'))
                        ...
                        <div class="form-group {{ $errors->has('message') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="message" id="message">{{ old('message', $pageForm->message) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('rgpd') ? 'has-error' : '' }}">
                            <div>
                                <input type="checkbox" name="rgpd" id="rgpd" value="1" {{ $pageForm->rgpd || old('rgpd', 0) === 1 ? 'checked' : '' }} required>
