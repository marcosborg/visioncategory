        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.productForm.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.product-forms.update", [$productForm->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.productForm.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $productForm->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone" id="phone" value="{{ old('phone', $productForm->phone) }}" required>
                            @if($errors->has('phone'))
                        ...
                        <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="email" name="email" id="email" value="{{ old('email', $productForm->email) }}">
                            @if($errors->has('email'))
                        ...
                        <div class="form-group {{ $errors->has('city') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="city" id="city" value="{{ old('city', $productForm->city) }}">
                            @if($errors->has('city'))
                        ...
                        <div class="form-group {{ $errors->has('rgpd') ? 'has-error' : '' }}">
                            <div>
                                <input type="checkbox" name="rgpd" id="rgpd" value="1" {{ $productForm->rgpd || old('rgpd', 0) === 1 ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('product') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="product_id" id="product_id" required>
                                @foreach($products as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $productForm->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
