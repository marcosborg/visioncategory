        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.contractTypeRank.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.contract-type-ranks.update", [$contractTypeRank->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('from') ? 'has-error' : '' }}">
                            <label for="from">{{ trans('cruds.contractTypeRank.fields.from') }}</label>
                            <input class="form-control" type="number" name="from" id="from" value="{{ old('from', $contractTypeRank->from) }}" step="0.01">
                            @if($errors->has('from'))
                        ...
                        <div class="form-group {{ $errors->has('to') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="to" id="to" value="{{ old('to', $contractTypeRank->to) }}" step="0.01">
                            @if($errors->has('to'))
                        ...
                        <div class="form-group {{ $errors->has('percent') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="percent" id="percent" value="{{ old('percent', $contractTypeRank->percent) }}" step="0.01" required>
                        ...
                        <div class="form-group {{ $errors->has('contract_type') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="contract_type_id" id="contract_type_id" required>
                                @foreach($contract_types as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('contract_type_id') ? old('contract_type_id') : $contractTypeRank->contract_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
