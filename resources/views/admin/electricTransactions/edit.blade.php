        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.electricTransaction.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.electric-transactions.update", [$electricTransaction->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('tvde_week') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="tvde_week_id" id="tvde_week_id" required>
                                @foreach($tvde_weeks as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('tvde_week_id') ? old('tvde_week_id') : $electricTransaction->tvde_week->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('card') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="card" id="card" value="{{ old('card', $electricTransaction->card) }}" required>
                            @if($errors->has('card'))
                        ...
                        <div class="form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="amount" id="amount" value="{{ old('amount', $electricTransaction->amount) }}" step="0.01" required>
                        ...
                        <div class="form-group {{ $errors->has('total') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total" id="total" value="{{ old('total', $electricTransaction->total) }}" step="0.01" required>
                            @if($errors->has('total'))
